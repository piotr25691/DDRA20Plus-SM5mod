...
t[#t+1] = Def.ActorFrame{
	...
	LoadActor(THEME:GetPathB("","_door/_backdrop"))..{
		InitCommand=cmd(zoom,1);
   	 OnCommand=cmd(sleep,0.2;accelerate,0.3;zoom,2.2;diffusealpha,0;);
	};
...
t[#t+1] = LoadActor( "ScreenGameplay out/"..ddrgame.."cleared" )..{
	InitCommand=cmd(zoom,1.5;x,SCREEN_CENTER_X;y,SCREEN_CENTER_Y;);
	OnCommand=cmd(linear,0.08;zoomy,0);
};

t[#t+1] = LoadActor( "ScreenGameplay out/"..ddrgame.."glow_cleared" )..{
