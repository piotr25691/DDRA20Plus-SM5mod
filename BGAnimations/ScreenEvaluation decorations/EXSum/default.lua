...
t[#t+1] = LoadActor("exback")..{
    InitCommand=function(s) s:xy(pn==PLAYER_1 and 34 or 15,64):rotationy(pn==PLAYER_1 and 0 or 180):zoomx(pn==PLAYER_1 and 1 or -1) end,
	OffCommand=cmd(linear,0.15;addx,-300);
};

t[#t+1] = LoadFont("_arial black 28px")..{
	InitCommand=function(self)
		self:xy(pn==PLAYER_1 and -58 or -78,40)
		self:align(pn==PLAYER_1 and 0 or 1,0):zoom(0.36):diffuse(color("#feec0a"))
		self:settext("GOLDEN LEAGUE\nTOTAL EX SCORE");
		self:zoomx(pn==PLAYER_1 and 0.35 or -0.35);
...
t[#t+1] = Def.RollingNumbers {
    ...
    InitCommand=function(self)
        ...
        self:xy(pn==PLAYER_1 and 58 or 133,76):halign(pn==PLAYER_1 and 1 or 0):zoom(0.88)
        :diffuse(color("#feec0a")):maxwidth(210);
		self:targetnumber(exbefore)
		self:queuecommand("Set")
		self:zoomx(pn==PLAYER_1 and 0.85 or -0.85)
	end;
