						...
						if scores[rival] then
							...
							if scores[rival]:GetScore()>1  then
								if scores[rival]:GetScore()==1000000 and topgrade=="Grade_Tier07" then
									self:LoadBackground(THEME:GetPathG("_grades/GradeDisplayEval","Tier01"));
									self:diffusealpha(1);
									self:zoom(0.25);
									self:addx(-6);
								else
									self:LoadBackground(THEME:GetPathG("_grades/GradeDisplayEval",ToEnumShortString(topgrade)));
									self:diffusealpha(1);
									self:zoom(0.25);
									self:addx(-6);
								end;	
...
t[#t+1] = LoadActor("ScoreFrame.png")..{
	InitCommand=cmd(player,PLAYER_1;vertalign,top;x,SCREEN_CENTER_X-245;y,SCREEN_CENTER_Y+70-30-20+5;zoom,0.7);
