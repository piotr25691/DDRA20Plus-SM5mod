...
local function MakeItem(sortorders, idx)
	return Def.ActorFrame{
		...
		OnCommand=function(s)
			if idx%2==0 then
				s:addx(200):diffusealpha(0):sleep(idx/120):decelerate(0):diffusealpha(1):addx(-200)
			else
				s:addx(-200):diffusealpha(0):sleep(idx/120):decelerate(0):diffusealpha(1):addx(200)
			end
		end,
		...
		Def.ActorFrame{
			...
			LoadActor(THEME:GetPathG("","_shared/cursor"))..{
				InitCommand=function(s) s:x(-90)
					s:bounce():effectclock("beat"):effectperiod(1):effectmagnitude(-10,0,0):effectoffset(0.5):zoom(0.75)
				end,
			},
			LoadActor(THEME:GetPathG("","_shared/cursor"))..{
				InitCommand=function(s) s:x(90):zoomx(-0.75):zoomy(0.75)
					s:bounce():effectclock("beat"):effectperiod(1):effectmagnitude(10,0,0):effectoffset(0.5)
				end,
			},
		},
		Def.BitmapText{
...
local t = Def.ActorFrame{
	...
	LoadActor("blue_Backer")..{
		OnCommand=function(s) s:xy(-6,38):zoomy(0):decelerate(0.2):zoomy(0.9) end,
	},
	Def.ActorFrame{
		OnCommand=function(s) s:x(-6):valign(1):y(0):decelerate(0.2):y(-208) end,
		LoadActor("blue_Header")..{
			InitCommand=function(s) s:y(-10) end,
		},
		LoadActor(lang.."_title.png")..{
			InitCommand=function(s) s:xy(38,-8) end,
		}
	};
	LoadActor("blue_Bottom")..{
		OnCommand=function(s) s:x(-6):y(0):decelerate(0.2):y(294) end,
	},
	Def.ActorScroller{
		SecondsPerItem=0;
		NumItemsToDraw=20;
